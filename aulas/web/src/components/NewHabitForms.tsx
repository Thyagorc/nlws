import { AlignCenterHorizontalSimple, Check } from "phosphor-react";
...
export function NewHabitForm(){
    ...
    function handleToggleWeekDay(weekDay: number) {
        if (weekDays.includes(weekDay)) {
            const WeekDaysWithRemovedOOne = weekDays.filter(day => day !== weekDay )

            setWeekdays(WeekDaysWithRemovedOOne)
        }   else {
            const WeekDaysWithRemovedOOne = [...weekDays, weekDay]
    ...
    return (
        <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
            ...
            <button 
            type="submit" 
            className="mt-6 rounded-lg p-4 flex justify-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors focus:outline-none focus:ring-2 focus:ring-green-700 focus:ring-offset-2 focus:ring-offset-zinc-900">
